image:
	docker build -t kune/docker-ddns:latest .

console:
	docker run -it -p 8080:8080 -p 53:53 -p 53:53/udp --rm kune/docker-ddns:latest bash

devconsole:
	docker run -it --rm -v ${PWD}/rest-api:/usr/src/app -w /usr/src/app golang:1.8.5 bash

server_test:
	docker run -it -p 8080:8080 -p 53:53 -p 53:53/udp --env-file envfile --rm kune/docker-ddns:latest
...
deploy: image
	docker run -it -d -p 8080:8080 -p 53:53 -p 53:53/udp --env-file envfile --name=dyndns kune/docker-ddns:latest
